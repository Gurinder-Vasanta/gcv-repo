...
targets = []
#3,6
temp = [1]*(int(total_sample_numbers/2))
t2 = [2]*(int(total_sample_numbers/2))
targets.append(temp)
targets.append(t2)
...
print(np.ndarray.tolist(pre.LabelBinarizer().fit_transform(categories)[0]))
print(sum(np.ndarray.tolist(pre.LabelBinarizer().fit_transform(categories)[0])))
input('asdjkfl')
#print(len(categories))
...
history = model.fit(encoded, targets, epochs=2, verbose=1,batch_size=125)
